...
double yearOfN = 1.9; //计算多少年的电费
double aOfCpu = 0.4;  //cpu所占权重
double threshold_cpu_memory = 2.3;
double threshold_memory_cpu = 1.73;
...
#ifdef LOCAL_DEBUG
	vector<string> file;
	file.push_back("/home/ubuntu/training-data/training-1.txt");
	file.push_back("/home/ubuntu/training-data/training-2.txt");
	ifstream ifs;
...
void ServerScheduling::migration(){
	...
	if( numOfMigration>0 && totalRatio<max_ratio ){
		...
		for(const auto &p : servers){
			...
			// 虚拟机链表copy一份，排序用
			vector<pair<int, string>> vms;		// vmId, vmType
...
bool cmp3(pair<int, string> &lhs, pair<int, string> &rhs){
	...
	int l_value = p_lhs->cpuSize + p_lhs->memorySize;
	int r_value = p_rhs->cpuSize + p_rhs->memorySize;
